...
2. **Invocação do Auto-Healing:** Chama a função ensureTripAtlasSchema() que está no vosso db.js.
...
4. **Injeção de Chaves Estrangeiras Inteligente:** Para evitar erros de duplicação, o vosso método ensureForeignKey faz uma consulta ao information_schema do MySQL para verificar se a restrição (ex: fk_trips_user) já existe. Se não existir, ele injeta o ALTER TABLE automaticamente com as regras de ON DELETE CASCADE.


## Quais são as Vantagens?
1. **Configuração Zero (Zero-Configuration Deployment):** Se um novo colega de equipa ou o professor descarregar o vosso código do GitHub, ele não precisa de importar um ficheiro .sql manualmente. Basta criar uma base de dados vazia, ligar o servidor, e o Node constrói o ecossistema inteiro sozinho de forma idêntica.
...
3. **Sincronização com as Camadas Superiores:** Garante que o vosso código JavaScript e a vossa Base de Dados estão sempre em sintonia com as mesmas restrições e colunas, evitando erros misteriosos de runtime.
